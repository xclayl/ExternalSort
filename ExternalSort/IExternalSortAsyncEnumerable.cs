...
internal class ExternalSortAsyncEnumerable<T, TK> : IExternalSortAsyncEnumerable<T> where T : new()
{
    ...
    internal ExternalSortAsyncEnumerable(IAsyncEnumerable<T> src, Func<T, TK> keySelector)
    {
        _src = src;
        _keySelector = keySelector;
    }
    
    private ExternalSortAsyncEnumerable(Func<T, long> calculateBytesInRam, int mbLimit, int openFilesLimit, IAsyncEnumerable<T> src, Func<T, TK> keySelector)
    {
    ...
    {
        return new ExternalSortAsyncEnumerable<T, TK>(calculateBytesInRam ?? _calculateBytesInRam, mbLimit ?? _mbLimit, openFilesLimit ?? _openFilesLimit, _src, _keySelector);
    }

    public async IAsyncEnumerator<T> GetAsyncEnumerator(CancellationToken cancellationToken = new())
    {
        using var sorter = new ExternalSorter<T, TK>(_calculateBytesInRam, _keySelector);
